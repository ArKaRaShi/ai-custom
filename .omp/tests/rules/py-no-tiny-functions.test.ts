...
describe("given py-no-tiny-functions rule, when parsing frontmatter and evaluating triggers, then validate schema and detect one-line wrappers", () => {
  const rulePath = path.resolve(__dirname, "../../rules/py-no-tiny-functions.md");
  const { frontmatter, body } = parseRule(rulePath);

  it("validates required frontmatter fields and body", () => {
    expect(frontmatter.description).toBe("Avoid Python functions that add no behavior");
  ...
  it("matches positive function definitions needing review", () => {
  ...
  it("does not match non-function statements or clean negatives", () => {
    const regexes = (frontmatter.condition ?? []).map(parseRegex);
